...
class FilterCollection implements \ArrayAccess, \Iterator, \Countable
{
    ...
    public function setFilterData($data)
    {
        foreach ($data as $index => $config) {
            ...
            $filter = new \SOCLITE\Detector\Filter($config);
     ...
     *
     * @param \SOCLITE\Detector\Filter $filter Filter object
     */
    public function addFilter(\SOCLITE\Detector\Filter $filter)
    {
